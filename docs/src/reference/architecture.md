...
- **WebGPU presentation boundary:** RAD exports a bounded, exact `u32` packet whose runtime-published descriptor binds every field offset and limit. The browser host owns all GPU devices, buffers, pipelines, command submission, resizing, and device-loss recovery. GPU handles never enter snapshots, replay, or causal state; the next packet rematerializes disposable resources after recovery. See [WebGPU Presentation Host](../guide/webgpu.md).
