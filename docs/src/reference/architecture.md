...
- **WebGPU presentation boundary:** RAD exports a bounded, exact `u32` packet whose runtime-published descriptor binds every field offset, limit, stream identity, sequence, and delta baseline. The browser host owns all GPU devices, buffers, pipelines, command submission, resizing, and generation-safe device-loss recovery. GPU handles never enter snapshots, replay, or causal state; the next full packet rematerializes disposable resources after recovery. See [WebGPU Presentation Host](../guide/webgpu.md).
