...
- **WASM / native integration:** Host/plugin code should still build external `Value`s using VM allocation APIs (`VM::gc_mut`) for host-visible handles.
- **Embeds / hosts:** Rust code that calls into the VM with fresh [`Value`](../../../core/vm/src/value.rs)s should allocate on the VM's heap via [`VM::gc_mut`](../../../core/vm/src/vm/mod.rs) (e.g. `Value::from_string(vm.gc_mut(), s)`).
